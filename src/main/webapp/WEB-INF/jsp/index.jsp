    ...
    <div class="row clearfix">
        <div class="col-md-12 column">
            <table class="table table-hover table-striped">
                ...
                <c:forEach var="booksList" items="${requestScope.get('booksList')}">
                    <tr>
                            <%-- setter 方法要和实体类中的 setter 方法一致 --%>
                                <td>${booksList.bid}</td>
                                <td>${booksList.bname}</td>
                                <td>${booksList.aname}</td>
                                <td>${booksList.bdescribe}</td>
                                <td>${booksList.tname}</td>
                        <td>
                            <%--权限--%>
                            <shiro:hasPermission name="user:update">
                                <a href="${pageContext.request.contextPath}/toUpdateBook?id=${booksList.bid}">更改</a>
                            </shiro:hasPermission>
                            <shiro:hasPermission name="user:delete">
                                <a href="${pageContext.request.contextPath}/deleteBook?bid=${booksList.bid}">删除</a>
                            </shiro:hasPermission>
                                <a href="${pageContext.request.contextPath}/looKooks?id=${booksList.bid}" id="about-dialog-button-bar">订阅</a>
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
